...
ParticleSystemSerial::ParticleSystemSerial(unsigned numParticles,
                                           glm::vec3 bounds_max,
                                           std::string config)
    ...
    for (int i = 0; i < numParticles; i++) {
      if (i % 2) {
        particles[i] = new Particle(
            glm::vec3(distributionX(generator), distributionY(generator),
                      distributionZ(generator)),
            maxNeighbors);
      } else {
        particles[i] = new Particle(
            glm::vec3(distributionZ(generator), distributionY(generator),
...
float *ParticleSystemSerial::getParticlePos() { return &particlePos[0].x; }
...
void ParticleSystemSerial::get_lambda() {
  for (auto i : particles) {
    ...
    for (auto j : i->neighbors) {
      ...
      // std::cout <<glm::to_string(spiky_prime(i->x_next -
      // j->x_next))<<","<<glm::to_string(i->x_next)<<","<<
      // glm::to_string(j->x_next)<< std::endl;
    }
