...
static const auto keyHash =
    [](const std::tuple<size_t, size_t, size_t> &k) -> size_t {
  return ((std::get<0>(k) * 73856093) + (std::get<1>(k) * 19349663) +
          (std::get<2>(k) * 83492791)) %
         200003;
};
static const auto keyEqual =
    [](const std::tuple<size_t, size_t, size_t> &lhs,
       const std::tuple<size_t, size_t, size_t> &rhs) -> bool {
  return (std::get<0>(lhs) == std::get<0>(rhs) &&
          std::get<1>(lhs) == std::get<1>(rhs) &&
          std::get<2>(lhs) == std::get<2>(rhs));
};

typedef std::unordered_multimap<std::tuple<size_t, size_t, size_t>, Particle *,
                                decltype(keyHash), decltype(keyEqual)>
    hashMap;
